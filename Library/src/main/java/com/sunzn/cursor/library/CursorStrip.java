...
public class CursorStrip extends LinearLayout {
    ...
    CursorStrip(Context context, AttributeSet attrs) {
        ...
        int indicationInterpolatorId = CursorInterpolator.ID_CURSOR;
        ...
        indicatorAlwaysInCenter = a.getBoolean(R.styleable.CursorView_cv_indicatorAlwaysInCenter, indicatorAlwaysInCenter);
        indicatorWithoutPadding = a.getBoolean(R.styleable.CursorView_cv_indicatorWithoutPadding, indicatorWithoutPadding);
        indicatorInFront = a.getBoolean(R.styleable.CursorView_cv_indicatorInFront, indicatorInFront);
        indicationInterpolatorId = a.getInt(R.styleable.CursorView_cv_indicatorInterpolation, indicationInterpolatorId);
        indicatorGravity = a.getInt(R.styleable.CursorView_cv_indicatorGravity, indicatorGravity);
        indicatorColor = a.getColor(R.styleable.CursorView_cv_indicatorColor, indicatorColor);
        indicatorColorsId = a.getResourceId(R.styleable.CursorView_cv_indicatorColors, indicatorColorsId);
        indicatorThickness = a.getDimensionPixelSize(R.styleable.CursorView_cv_indicatorThickness, indicatorThickness);
        indicatorWidth = a.getLayoutDimension(R.styleable.CursorView_cv_indicatorWidth, indicatorWidth);
        indicatorCornerRadius = a.getDimension(R.styleable.CursorView_cv_indicatorCornerRadius, indicatorCornerRadius);
        overlineColor = a.getColor(R.styleable.CursorView_cv_overLineColor, overlineColor);
        overlineThickness = a.getDimensionPixelSize(R.styleable.CursorView_cv_overLineThickness, overlineThickness);
        underlineColor = a.getColor(R.styleable.CursorView_cv_underLineColor, underlineColor);
        underlineThickness = a.getDimensionPixelSize(R.styleable.CursorView_cv_underLineThickness, underlineThickness);
        dividerColor = a.getColor(R.styleable.CursorView_cv_dividerColor, dividerColor);
        dividerColorsId = a.getResourceId(R.styleable.CursorView_cv_dividerColors, dividerColorsId);
        dividerThickness = a.getDimensionPixelSize(R.styleable.CursorView_cv_dividerThickness, dividerThickness);
        drawDecorationAfterTab = a.getBoolean(R.styleable.CursorView_cv_drawDecorationAfterTab, drawDecorationAfterTab);
        a.recycle();
        ...
        this.indicationInterpolator = CursorInterpolator.of(indicationInterpolatorId);
    }
    ...
    void setIndicationInterpolator(CursorInterpolator interpolator) {
    ...
    private void drawDecoration(Canvas canvas) {
        ...
        if (tabCount > 0) {
            ...
            int selectedStart = CursorUtils.getStart(selectedTab, indicatorWithoutPadding);
            int selectedEnd = CursorUtils.getEnd(selectedTab, indicatorWithoutPadding);
            int left;
            int right;
            if (isLayoutRtl) {
                left = selectedEnd;
            ...
            if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
                ...
                int nextStart = CursorUtils.getStart(nextTab, indicatorWithoutPadding);
                int nextEnd = CursorUtils.getEnd(nextTab, indicatorWithoutPadding);
                if (isLayoutRtl) {
    ...
    private void drawSeparator(Canvas canvas, int height, int tabCount) {
        ...
        for (int i = 0; i < tabCount - 1; i++) {
            ...
            int end = CursorUtils.getEnd(child);
            int endMargin = CursorUtils.getMarginEnd(child);
    ...
    private static class SimpleTabColorizer implements CursorView.TabColorizer {
