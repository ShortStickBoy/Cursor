        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (layoutManager != null && getScrollState() != SCROLL_STATE_DRAGGING) {
                    ...
                    if (canScrollHorizontally && Math.abs(dx) > mTouchSlop && (Math.abs(dx) * 0.5f >= Math.abs(dy) || canScrollVertically)) {
                        startScroll = true;
                    }

                    if (canScrollVertically && Math.abs(dy) > mTouchSlop && (Math.abs(dy) * 0.5f >= Math.abs(dx) || canScrollHorizontally)) {
                        startScroll = true;
                    }
